...
fileID = fopen('modelo_webe4.bin','w');
fwrite(fileID,modelo,'single');
fclose(fileID);

modelo(size(modelo,1)/2-50,size(modelo,2)/2)=1800;

fileID = fopen('modelo_webe4_real.bin','w');
fwrite(fileID,modelo,'single');
fclose(fileID);
...
fileID = fopen('ref_modelo_webe4.bin','w');
fwrite(fileID,ref,'single');
fclose(fileID)

back=ones(size(modelo))*1500;
true_ref=modelo-back;
fileID = fopen('trueref_modelo_webe4.bin','w');
...
xt = get(gca, 'XTick');
set(gca, 'FontSize', 14)
title('Modelo Webe 4','interpreter','latex','fontsize',16); 
...
xt = get(gca, 'XTick');
set(gca, 'FontSize', 14)
title('Refletividade - Modelo Webe 4','interpreter','latex','fontsize',16); 
xlabel('(m)', 'FontSize', 14,'Interpreter','latex'); % x-axis label
ylabel('(m)', 'FontSize',14,'Interpreter','latex'); % y-axis label
